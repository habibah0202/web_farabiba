        ...
        <div class="min-h-screen bg-[#f6f3ee] text-[#2a211b]">
            <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                ...
                <div class="mb-6">
                    ...
                    <h1 class="mt-2 font-serif text-4xl text-[#2d241d]">Kategori Produk</h1>
                </div>

                <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                @foreach ($categories as $category)
                    <div class="rounded-[1.6rem] border border-[#e7d9cb] bg-[#f9f4ef] p-5 shadow-sm">
                        <div class="mb-3 text-[10px] font-semibold uppercase tracking-[0.25em] text-[#7b675d]">{{ $category->products_count }} produk</div>
                        <h2 class="font-serif text-3xl text-[#2d241d]">{{ $category->name }}</h2>
                        <p class="mt-3 text-sm leading-6 text-[#5d4d45]">Temukan rangkaian produk skincare terbaik untuk kebutuhan kulit Anda.</p>
                        <a href="{{ route('customer.filter', ['category_id' => $category->id]) }}" class="mt-5 inline-block rounded-full bg-[#2d241d] px-4 py-2 text-sm font-medium text-white">Lihat produk</a>
                    </div>
                @endforeach
            </div>
        </div>
