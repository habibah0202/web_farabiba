    ...
    <body class="bg-[#f6f3ee] text-[#2a211b] antialiased">
        <div class="min-h-screen bg-[#f6f3ee] text-[#2a211b]">
            ...
            <div class="mx-auto max-w-6xl px-4 pb-8 sm:px-6 lg:px-8">
                <div class="grid gap-8 rounded-[2rem] border border-[#e7d9cb] bg-[#f9f4ef] p-6 shadow-sm lg:grid-cols-[0.9fr_1.1fr]">
                <div class="flex min-h-[300px] items-center justify-center rounded-[1.7rem] border border-[#eadcc9] bg-[#f3ebdf]">
                    <div class="text-center">
                        <div class="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-[#2d241d] text-3xl font-semibold text-[#f7efe9]">G</div>
                        <div class="mt-4 font-serif text-3xl text-[#2d241d]">{{ $product->name }}</div>
                    </div>
                </div>

                <div>
                    <span class="rounded-full bg-white px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#7b675d]">{{ $product->category?->name ?? 'Skincare' }}</span>
                    <h1 class="mt-4 font-serif text-5xl text-[#2d241d]">{{ $product->name }}</h1>
                    <p class="mt-4 text-lg font-semibold text-[#2d241d]">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                    <p class="mt-4 text-base leading-7 text-[#5d4d45]">{{ $product->description }}</p>

                    <div class="mt-6 flex items-center gap-4">
                        <span class="rounded-full bg-[#efe0d5] px-3 py-1 text-sm font-medium text-[#5d4d45]">Stok: {{ $product->stock }}</span>
                        <span class="rounded-full bg-[#efe0d5] px-3 py-1 text-sm font-medium text-[#5d4d45]">Status: {{ $product->status === 'active' ? 'Aktif' : 'Nonaktif' }}</span>
                    </div>

                    <div class="mt-8 flex flex-wrap gap-4">
                        <form action="{{ route('customer.products.addToCart', $product) }}" method="POST">
                            @csrf
                            <button type="submit" class="rounded-full bg-[#2d241d] px-6 py-3 text-sm font-medium text-white">Tambah ke Keranjang</button>
                        </form>
                        <a href="{{ route('customer.filter') }}" class="rounded-full border border-[#d9c7b5] bg-white px-6 py-3 text-sm font-medium text-[#2d241d]">Lanjut Belanja</a>
                    </div>
                </div>
            </div>
        </div>
