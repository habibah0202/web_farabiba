...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-[#f6f3ee] text-[#2a211b] antialiased">
        <div class="min-h-screen bg-[#f6f3ee] text-[#2a211b] antialiased">
            <header class="border-b border-[#e5d8cb] bg-[#f6f3ee]">
            ...
            <div class="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.32em] text-[#7d685f]">Filter</p>
                    <h1 class="mt-2 font-serif text-4xl text-[#2d241d]">Filter Produk</h1>
                </div>

                <form method="GET" action="{{ route('customer.filter') }}" class="mb-6 flex flex-col gap-3 rounded-[1.6rem] border border-[#e7d9cb] bg-[#f9f4ef] p-5 md:flex-row md:items-center">
                    <select name="category_id" class="w-full rounded-full border border-[#d9c7b5] bg-white px-4 py-3 text-sm text-[#2d241d] focus:border-[#2d241d] focus:outline-none md:w-72">
                        <option value="">Semua kategori</option>
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" {{ (string) $categoryId === (string) $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                    <button type="submit" class="rounded-full bg-[#2d241d] px-5 py-3 text-sm font-medium text-white">Terapkan</button>
                </form>
