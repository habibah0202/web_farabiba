    ...
    <body class="bg-[#f6f3ee] text-[#2a211b] antialiased">
        <div class="min-h-screen bg-[#f6f3ee] text-[#2a211b]">
            ...
            <div class="mx-auto max-w-6xl px-4 pb-8 sm:px-6 lg:px-8">
                <div class="mb-6">
                    <p class="text-[10px] font-semibold uppercase tracking-[0.32em] text-[#7d685f]">Keranjang</p>
                    <h1 class="mt-2 font-serif text-4xl text-[#2d241d]">Keranjang Belanja</h1>
                </div>
                            ...
                            <div class="flex flex-col gap-4 rounded-[1.6rem] border border-[#e7d9cb] bg-[#f9f4ef] p-5 shadow-sm md:flex-row md:items-center md:justify-between">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <form method="POST" action="{{ route('customer.cart.remove', $item['id']) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="rounded-full bg-red-100 px-3 py-2 text-sm font-medium text-red-700">Hapus</button>
                                    </form>
                                </div>
                            </div>
                        @endforeach
