<x-app-layout>
    <div class="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <div class="mb-6">
            ...
            <h1 class="mt-2 font-serif text-4xl text-[#2d241d]">Edit Produk</h1>
        </div>

        <form action="{{ route('products.update', $product) }}" method="POST" class="rounded-[2rem] border border-[#eadcc9] bg-white p-6 shadow-sm">
            @csrf
            @method('PUT')
            <div class="grid gap-5 md:grid-cols-2">
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-[#3f3129]">Nama Produk</label>
                    <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full rounded-xl border border-[#e1d2c0] bg-[#faf7f4] px-3 py-2.5 focus:border-[#2d241d] focus:outline-none" required>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $product->price) }}" class="w-full rounded-xl border border-[#e1d2c0] bg-[#faf7f4] px-3 py-2.5 focus:border-[#2d241d] focus:outline-none" required>
                ...
                <div>
                    ...
                    <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" class="w-full rounded-xl border border-[#e1d2c0] bg-[#faf7f4] px-3 py-2.5 focus:border-[#2d241d] focus:outline-none" required>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" class="w-full rounded-xl border border-[#e1d2c0] bg-[#faf7f4] px-3 py-2.5 focus:border-[#2d241d] focus:outline-none">{{ old('description', $product->description) }}</textarea>
            ...
            <div class="mt-6 flex justify-end gap-3">
                ...
                <button type="submit" class="rounded-full bg-[#2d241d] px-5 py-2.5 text-sm font-medium text-[#f8efe8]">Update Produk</button>
            </div>
        </form>
    </div>
</x-app-layout>
